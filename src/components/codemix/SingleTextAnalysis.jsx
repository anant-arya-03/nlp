...
import { Send, Zap, MessageSquareQuote, ShieldAlert, AlertCircle, Smile, Lightbulb } from 'lucide-react';
...
const SAMPLE_TEXTS = [
  "Bhaaiii kya mast vibe hai yaar 😭🔥 yeh movie literally fire hai bro no cap!",
  "omg wtf is this service smh 🤦🏽‍♂️ mera khana abhi tak nahi aaya... bahut kharab tbh 😡",
  "Ye naya update itna sus lag raha hai 🤡 fix karne wala koi nahi hai kya? pls lmk asap fr fr 🙏",
  "Aaj ka din soooo good tha! shaam ko chaiwala ke paas gaye, ekdum jhakkas maza aa gaya 😂✨",
  "kya bakwaas game hai yaar, bilkul ghatiya graphics the 💀 refund chahiye mujhe rn bro",
];
export default function SingleTextAnalysis() {
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      if (modelType === 'all' || modelType === 'smart') {
        normalizedResult = res.data;
      } else if (modelType === 'text') {
        normalizedResult = { text_analysis: res.data.text_analysis };
      } else if (modelType === 'emosen') {
        const emosenData = res.data.emosen ?? res.data;
        normalizedResult = {
          text_analysis: res.data.text_analysis ?? null,
          emosen: emosenData,
          routed_to: res.data.routed_to ?? 'emosen',
        };
      } else if (modelType === 'misinfo') {
        const misinfoData = res.data.misinfo ?? res.data;
        normalizedResult = {
          text_analysis: res.data.text_analysis ?? null,
          misinfo: misinfoData,
          routed_to: res.data.routed_to ?? 'misinfo',
        };
      } else if (modelType === 'fakenews') {
        const fakenewsData = res.data.fakenews ?? res.data;
        normalizedResult = {
          text_analysis: res.data.text_analysis ?? null,
          fakenews: fakenewsData,
          routed_to: res.data.routed_to ?? 'fakenews',
        };
      } else {
        const { text_analysis, ...modelData } = res.data;
        normalizedResult = { text_analysis, [modelType]: modelData };
      }
  ...
  const getAvailableTabs = () => {
    if (!result) return ['Model Result', 'Scripts & Languages', 'Slang Detected', 'Phoneme Hints', 'Text Stats'];
    const tabs = [];
    if (modelType !== 'text' && (result.misinfo || result.fakenews || result.emosen)) {
      tabs.push('Model Result');
    }
    if (result.text_analysis) {
      tabs.push('Scripts & Languages', 'Slang Detected', 'Phoneme Hints', 'Text Stats');
    }
    return tabs;
  };
  ...
  const hasModelResult = result && (
    (result.misinfo && !result.misinfo.error) ||
    (result.fakenews && !result.fakenews.error) ||
    (result.emosen && !result.emosen.error)
  );
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white rounded-xl shadow-card border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 flex-1 flex flex-col gap-4">
          <textarea
            ...
            className="w-full flex-1 min-h-[160px] bg-white border border-slate-300 rounded-xl p-5 text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 resize-none font-medium shadow-inner"
          />
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-amber-700">Sample texts to try:</span>
            </div>
            <div className="flex flex-col gap-2">
              {SAMPLE_TEXTS.map((sample, i) => (
                <button
                  ...
                  className="text-left text-sm bg-white border border-amber-100 hover:border-indigo-300 hover:bg-indigo-50 text-slate-600 hover:text-indigo-700 px-4 py-2.5 rounded-lg cursor-pointer transition-all duration-150 shadow-sm font-medium"
                >
                  "{sample}"
                </button>
              ))}
          ...
          {error && (
            <div className="text-rose-600 bg-rose-50 border border-rose-200 px-4 py-3 rounded-lg font-bold flex items-center shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!result ? (
            ...
            <div className="flex flex-col h-full">
              ...
              {/* Tab Content */}
              <div className="flex-1 bg-white rounded-xl border border-slate-200 p-5 shadow-sm overflow-y-auto">
                          ...
                          {Array.isArray(result.text_analysis.slang_analysis.internet_slang)
                            ...
                            : Object.entries(result.text_analysis.slang_analysis.internet_slang || {}).map(([s, meaning], idx) => (
                              <div key={idx} className="bg-blue-50 border border-blue-200 rounded-lg p-2 flex flex-col shadow-sm">
                                <span className="font-bold text-blue-800 text-sm">{s}</span>
                                <span className="text-xs text-blue-600 mt-1 flex items-center"><span className="text-blue-300 mr-1">↳</span> {meaning}</span>
                              </div>
                            ))
                          }
                          ...
                          {Array.isArray(result.text_analysis.slang_analysis.hinglish_slang)
                            ...
                            : Object.entries(result.text_analysis.slang_analysis.hinglish_slang || {}).map(([s, meaning], idx) => (
                              <div key={idx} className="bg-orange-50 border border-orange-200 rounded-lg p-2 flex flex-col shadow-sm">
                                <span className="font-bold text-orange-800 text-sm">{s}</span>
                                <span className="text-xs text-orange-600 mt-1 flex items-center"><span className="text-orange-300 mr-1">↳</span> {meaning}</span>
                              </div>
                            ))
                          }
